...
export class EventService {
  private eventsMock: EventIn[] = [
    {
      ...
      description:
        'Una conferencia dedicada a Angular, buenas prácticas, signals, y performance.',
      date: '2025-08-10',
      time: '10:00',
    ...
    },
    {
      ...
      description:
        'Un taller práctico para mejorar tus habilidades de diseño e interfaces accesibles.',
      date: '2025-08-22',
      time: '14:00',
    ...
    },
    {
      ...
      description:
        'Evento musical con bandas locales, foodtrucks y buena vibra.',
      date: '2025-09-01',
      time: '18:00',
    ...
    },
    {
      ...
      description:
        'Espacio para compartir ideas, buscar socios y potenciar startups tecnológicas.',
      date: '2025-08-15',
      time: '19:00',
    ...
    },
    {
      ...
      description:
        'Un vistazo a las últimas tendencias de la IA, LLMs y agentes autónomos.',
      date: '2025-08-30',
      time: '16:30',
      location: 'Online',
  ...
  constructor(
    private readonly httpC: HttpClient,
  ...
  getEventsByOrganizer(): Observable<EventIn[]> {
    // return this.httpC.get<any>(environment.apiUrl);
    if (!this.currentUserSignal()) {
      return of([]);
    }
    console.log('Organizer', this.currentUserSignal()?.id);
    const filteredEvents = this.eventsMock.filter(
      (event) => event.idOrganizer === this.currentUserSignal()?.id
    );
    return of([...filteredEvents]).pipe(delay(1000));
  }
  ...
  getEventById(id: string): Observable<EventIn> {
    ...
    if (!foundEvent) {
      return throwError(
        () => new Error('No se encontró el evento con el ID proporcionado')
      );
    }
  ...
  addEvent(event: EventIn): Observable<EventIn> {
    if (!this.currentUserSignal()) {
      return throwError(
        () => new Error('Debes estar logueado para crear un evento')
      );
    }
    ...
    const newEvent: EventIn = {
      ...event,
      id: uuid(),
      idOrganizer: this.currentUserSignal()!.id,
    };
  ...
  editEvent(event: EventIn): Observable<EventIn> {
    if (!event || !event.id) {
      return throwError(
        () =>
          new Error('No se pudo editar el evento. Evento inválido: falta el id')
      );
    }
